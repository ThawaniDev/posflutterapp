...
|---|--------|------|--------|--------|
| 2A.1 | `PosButton` | `pos_button.dart` | Add `disabled` opacity, hover animation, focus ring, RTL icon flip | ⬜ |
| 2A.2 | `PosTextField` | `pos_input.dart` | Add `isRequired` asterisk, `dense` variant, consistent error styling | ⬜ |
| 2A.3 | `PosSearchField` | `pos_input.dart` | Add debounce, loading state, width constraint | ⬜ |
| 2A.4 | `PosDropdown` | `pos_input.dart` | Dark mode fix, consistent border styling | ⬜ |
| 2A.5 | `PosToggle` | `pos_input.dart` | Dark mode text color fix | ⬜ |
| 2A.6 | `PosCheckboxTile` | `pos_input.dart` | Dark mode text fix, add `dense` mode | ⬜ |
| 2A.7 | `PosCard` | `pos_card.dart` | Add `variant` (elevated, outlined, flat), hover effect | ⬜ |
| 2A.8 | `PosKpiCard` | `pos_card.dart` | Filament-style clean card, consistent trend badge | ⬜ |
| 2A.9 | `PosDataTable` | `pos_table.dart` | Filament table: clean rows, no heavy borders, sticky header, density | ⬜ |
| 2A.10 | `PosBadge` | `pos_badge.dart` | Dark mode neutral variant fix | ⬜ |
| 2A.11 | `PosStatusBadge` | `pos_status_badge.dart` | Use PosCard dark mode pattern | ⬜ |
| 2A.12 | `PosEmptyState` | `pos_empty_state.dart` + `pos_scaffold.dart` | Remove duplicate, unify into one | ⬜ |
| 2A.13 | `PosErrorState` | `pos_error_state.dart` | Use PosButton instead of raw OutlinedButton | ⬜ |
| 2A.14 | `PosLoading` | `pos_scaffold.dart` | Dark mode text fix | ⬜ |
| 2A.15 | `PosDialog` | `pos_dialog.dart` | Dark mode message color, snackbar enhancement | ⬜ |
| 2A.16 | `PosSidebar` | `pos_sidebar.dart` | Filament-clean sidebar: minimal icons, clean hover, group headers | ⬜ |
| 2A.17 | `PosAppBar` | `pos_app_bar.dart` | Clean breadcrumb, responsive, profile dropdown | ⬜ |
| 2A.18 | `PosSearchableDropdown` | `pos_searchable_dropdown.dart` | Ensure consistent theming | ⬜ |
| 2A.19 | `PosLoadingSkeleton` | `pos_loading_skeleton.dart` | Table skeleton variant, card skeleton variant | ⬜ |

### 2B — New Widgets to Create

| # | Widget | Purpose | Status |
|---|--------|---------|--------|
| 2B.1 | `PosPageHeader` | Unified page header with title, breadcrumb, actions — Filament-style | ⬜ |
| 2B.2 | `PosFilterBar` | Horizontal filter chips/dropdowns bar for table pages | ⬜ |
| 2B.3 | `PosTabs` | Consistent tab bar for form/detail pages (Filament-style underline) | ⬜ |
| 2B.4 | `PosSection` | Labeled section wrapper with title + optional action (for forms) | ⬜ |
| 2B.5 | `PosStatsGrid` | Responsive KPI grid that auto-adapts columns | ⬜ |
| 2B.6 | `PosFormCard` | Form section card with title, description, and child form fields | ⬜ |
| 2B.7 | `PosDateRangePicker` | Themed date range picker for reports | ⬜ |
| 2B.8 | `PosAvatar` | User avatar with initials fallback and online dot | ⬜ |
| 2B.9 | `PosDivider` | Themed divider with optional label | ⬜ |
| 2B.10 | `PosProgressBar` | Linear progress with label and percentage | ⬜ |
| 2B.11 | `PosBreadcrumb` | Breadcrumb navigation trail | ⬜ |
| 2B.12 | `PosInfoBanner` | Dismissible info/warning/upgrade banner (Filament notification style) | ⬜ |
| 2B.13 | `PosStockDot` | Colored dot for stock status | ⬜ |
| 2B.14 | `PosCountBadge` | Notification count overlay | ⬜ |
| 2B.15 | `PosListPage` | Standard list page scaffold: header + search + filters + table | ⬜ |
| 2B.16 | `PosFormPage` | Standard form page scaffold: header + tabs + form card sections | ⬜ |
| 2B.17 | `PosDashboardPage` | Standard dashboard page scaffold: header + KPIs + charts grid | ⬜ |
| 2B.18 | `PosProductCard` | Product tile for POS grid | ⬜ |
| 2B.19 | `PosProductListCard` | Horizontal product card for list views | ⬜ |
| 2B.20 | `PosTimelineTile` | Timeline item for activity logs | ⬜ |
| 2B.21 | `PosChipGroup` | Group of selectable chips (single/multi) | ⬜ |
| 2B.22 | `PosColorPicker` | Simple color picker for customization | ⬜ |

---

## Phase 3: Feature Page Refactoring

### 3.1 Auth (3 pages)
| # | Page | File | Status |
|---|------|------|--------|
| 3.1.1 | Login | `auth/pages/login_page.dart` | ⬜ |
...
| # | Page | File | Status |
|---|------|------|--------|
| 3.2.1 | POS Cashier | `pos_terminal/pages/pos_cashier_page.dart` | ⬜ |
| 3.2.2 | Close Shift | `pos_terminal/pages/pos_close_shift_dialog.dart` | ⬜ |
...
| 3.3.2 | Product Form | `catalog/pages/product_form_page.dart` | ⬜ |
...
| 3.7.6 | Gift Cards | `payments/pages/gift_cards_page.dart` | ⬜ |
| 3.7.7 | Installment Payment | `payments/pages/installment_payment_dialog.dart` | ⬜ |
| 3.7.8 | Installment WebView | `payments/pages/installment_webview_page.dart` | ⬜ |

### 3.8 Reports (10 pages + 3 widgets)
| # | Page | File | Status |
|---|------|------|--------|
| 3.8.1 | Category Breakdown | `reports/pages/category_breakdown_page.dart` | ⬜ |
| 3.8.2 | Customer Report | `reports/pages/customer_report_page.dart` | ⬜ |
| 3.8.3 | Dashboard | `reports/pages/dashboard_page.dart` | ⬜ |
| 3.8.4 | Financial Report | `reports/pages/financial_report_page.dart` | ⬜ |
| 3.8.5 | Hourly Sales | `reports/pages/hourly_sales_page.dart` | ⬜ |
| 3.8.6 | Inventory Report | `reports/pages/inventory_report_page.dart` | ⬜ |
| 3.8.7 | Payment Methods | `reports/pages/payment_methods_page.dart` | ⬜ |
| 3.8.8 | Product Performance | `reports/pages/product_performance_page.dart` | ⬜ |
| 3.8.9 | Sales Summary | `reports/pages/sales_summary_page.dart` | ⬜ |
| 3.8.10 | Staff Performance | `reports/pages/staff_performance_page.dart` | ⬜ |
...
| 3.8.13 | Report Widgets | `reports/widgets/report_widgets.dart` | ⬜ |
...
| 3.11.7 | Staff Detail | `staff/pages/staff_detail_page.dart` | ⬜ |
...
| 3.12.2 | Branch Form | `branches/pages/branch_form_page.dart` | ⬜ |
