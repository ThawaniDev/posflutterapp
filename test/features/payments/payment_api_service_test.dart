...
Map<String, dynamic> _env(dynamic data, {String message = 'ok'}) => {'success': true, 'message': message, 'data': data};

/// Paginated envelope.
Map<String, dynamic> _page(List<dynamic> items, {int total = 0, int page = 1, int lastPage = 1, int perPage = 20}) => _env({
  'data': items,
...
void main() {
  ...
  group('PaymentApiService.createPayment', () {
    ...
    test('throws DioException on server error', () async {
      ...
      expect(() => svc.createPayment({'transaction_id': 'bad', 'method': 'cash', 'amount': 0}), throwsA(isA<DioException>()));
    });
  });
    ...
    test('sends GET without date param when null', () async {
      final svc = PaymentApiService(
        _fakeDio((opts) {
          ...
          return _env({
            'revenue': {'gross': 0.0, 'refunds': 0.0, 'expenses': 0.0, 'net': 0.0},
