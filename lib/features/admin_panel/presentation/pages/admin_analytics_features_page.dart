...
class _AdminAnalyticsFeaturesPageState extends ConsumerState<AdminAnalyticsFeaturesPage> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() => ref.read(analyticsFeaturesProvider.notifier).load());
  }
    ...
    return Scaffold(
      ...
      body: switch (state) {
        ...
        AnalyticsFeaturesLoaded(features: final features, trend: final trend) => RefreshIndicator(
          onRefresh: () => ref.read(analyticsFeaturesProvider.notifier).load(),
          child: ListView(
        ...
        AnalyticsFeaturesError(message: final msg) => Center(
          child: Column(
            ...
            children: [
              Text('Error: $msg'),
              const SizedBox(height: AppSpacing.sm),
              ElevatedButton(onPressed: () => ref.read(analyticsFeaturesProvider.notifier).load(), child: const Text('Retry')),
            ],
          ),
        ),
